...
B-01 fix (indirect): parse_scope_csv() now exists in csv_parser.py.
B-08 fix: calls db.set_db_path(out_root) so history DB follows --output flag.
"""
import argparse
import asyncio
import hashlib
...
def print_summary(aggregated: dict, new_findings: list, run_id: str, out_dir: Path) -> None:
    ...
    for domain, sig in aggregated.get("domain_signals", {}).items():
        ...
        boost_note = f" (+{sig.get('nmap_boost', 0)} nmap)" if sig.get("nmap_boost") else ""
        print(f"  {icon}  {domain:<35} score={sig.get('interest_score', 0):<6}{boost_note} {p}")
    print("=" * 60 + "\n")
...
async def main():
    ...
    # --history must not require config.yaml (B-04 from prev audit)
    if args.history:
    ...
    targets = parse_scope_csv(Path(args.csv))  # B-01 fix: function now exists
    ...
    # B-08 fix: DB follows --output flag
